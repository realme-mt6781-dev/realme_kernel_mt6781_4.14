...
#define CHARGER_SUBTYPE_FASTCHG_SVOOC_65W 20
#define CHARGER_SUBTYPE_FASTCHG_SVOOC_33W 97
...
//add by lukaili notify fuelgauge for other charger
			...
			if (odm_select_bat_ntc_support == ODM_SPACE_B_33W) {
				...
				if (noti->typec_state.old_state == TYPEC_ATTACHED_SNK && noti->typec_state.new_state == TYPEC_UNATTACHED) {
					if(oplus_vooc_get_fastchg_started() && oplus_vooc_get_fast_chg_type() == CHARGER_SUBTYPE_FASTCHG_VOOC
					&& oplus_chg_get_charger_voltage() >= LOW_CHARGER_VBUS_FLAG){
						msleep(500);
						pr_err("vooc status %s %d %d %d %d\n", __func__, __LINE__, oplus_chg_get_charger_voltage(), oplus_vooc_get_fastchg_started(), oplus_vooc_get_fast_chg_type());
						if (oplus_vooc_get_fastchg_started() && oplus_vooc_get_fast_chg_type() == CHARGER_SUBTYPE_FASTCHG_VOOC
							&& oplus_chg_get_charger_voltage() >= LOW_CHARGER_VBUS_FLAG) {
							vbus_check_charger_plug_out = true;
							return NOTIFY_OK;
						}
					} else if((oplus_vooc_get_fast_chg_type() == CHARGER_SUBTYPE_FASTCHG_SVOOC_65W || oplus_vooc_get_fast_chg_type() == CHARGER_SUBTYPE_FASTCHG_SVOOC_33W)
							&& (oplus_vooc_get_fastchg_to_normal() || oplus_chg_get_flash_led_status())){
						msleep(2000);
						pr_err("svooc status %s %d %d %d %d %d %d\n", __func__, __LINE__, oplus_chg_get_charger_voltage(), oplus_vooc_get_fastchg_started(), oplus_vooc_get_fast_chg_type(), oplus_vooc_get_fastchg_to_normal(), oplus_chg_get_flash_led_status());
						if ((oplus_vooc_get_fast_chg_type() == CHARGER_SUBTYPE_FASTCHG_SVOOC_65W || oplus_vooc_get_fast_chg_type() == CHARGER_SUBTYPE_FASTCHG_SVOOC_33W)
							&& oplus_vooc_get_fastchg_started() == false && oplus_chg_get_charger_voltage() >= SVOOC_FASTCHG_TO_NORMAL_VBUS) {
							vbus_check_charger_plug_out = true;
							return NOTIFY_OK;
						}
					}
				...
				if (noti->typec_state.old_state == TYPEC_ATTACHED_NORP_SRC && noti->typec_state.new_state == TYPEC_UNATTACHED && oplus_vooc_get_fastchg_started()) {
					pr_err("charger status old_state TYPEC_ATTACHED_NORP_SRC %s %d %d\n", __func__, __LINE__, oplus_vooc_get_fastchg_started());
					vbus_check_charger_plug_out = true;
					return NOTIFY_OK;
				}
			}
